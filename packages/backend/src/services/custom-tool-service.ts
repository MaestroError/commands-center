...
function renderToolTemplate(name: string, description: string): string {
  return [
    ...
    "",
    "// External actions (webhooks, uploads, image generation, n8n, …) routinely take",
    "// longer than a few seconds, so outbound requests default to a 1-minute timeout.",
    "// Raise REQUEST_TIMEOUT_MS for this tool if it calls a slower endpoint.",
    ...
    "  args: {},",
    "  async execute() {",
    "    // TODO: point this at your endpoint and return its result.",
    '    const endpoint: string = ""; // e.g. "https://example.com/webhook"',
    "    if (!endpoint) {",
