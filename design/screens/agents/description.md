...
Agents is the main screen for browsing and selecting all available agents in CommandsCenter. It should help the single operator quickly find an agent, understand its role, and move into the next action without relying on the sidebar alone.
...
- As a single user, I want to see all available agents in one place, so that I can choose the right agent without searching through navigation shortcuts.
- As a single user, I want to search agents by name or role, so that I can find the agent I need when the list grows.
