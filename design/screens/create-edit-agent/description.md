...
- Let the user control which globally configured tools and MCP servers the agent can use, including per-tool or per-server permission behavior when supported.
...
- As a single user, I want to control which tools and MCP integrations an agent can access, so that each agent only receives the capabilities I intend.
