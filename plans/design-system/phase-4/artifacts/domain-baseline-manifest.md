...
| Settings/API/tools     | custom-tools E2E + Checkbox gallery test                      | dense actions, destructive flow, tri-state                  | light/dark primitive assertions; narrow dialog bounds    |
...
State coverage is split intentionally: primitive/common fixtures own focus,
disabled, selected, success/warning/danger, dialogs, menus, and narrow bounds;
domain E2E owns real data/API/navigation behavior. Appearance assertions cover
semantic-token/icon/primitive migration and the shell status dot documented by
DS-0402.
