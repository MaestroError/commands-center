...
| Unclassed content reaches 615px inside a 328px content box             | Phase 1                    | Semantic rollout must include long-token/pre/table overflow in its final batch                                                              | Generic semantic style/overflow assertions, `.cc-md` unchanged                                                   |
...
| Current dialogs lack a unified accessible behavior contract            | Phases 2–3                 | First Radix batch is Button/Dialog/AlertDialog; add focus, Escape, portal, and safe-action tests before consumer migration                  | Focus in/return, accessible title/description, overlay/Escape, callbacks, semantic surface checks                |
...
| Phase 0 adds deterministic appearance fixtures                         | Phases 1–6                 | Preserve semantic, layout, behavior, and manual-review coverage without committed screenshot baselines                                      | Two deterministic runs for every affected batch                                                                  |
...
- `system` is not a third palette and requires no third appearance fixture set.
